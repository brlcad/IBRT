...
- Host: `Miniagua.local`, `Darwin arm64` (metadata.txt)
- Scene: default startup demo model `moss.g`
- Artifacts:
  - Baseline report: report.md
  - UI flame graph: IBRT.svg
  - Worker flame graph: IBRTRenderWorker.svg
  - Worker raw sample: IBRTRenderWorker.sample.txt
...
- The UI process was mostly idle during the capture. `mach_msg2_trap` accounted for `90.5%` of UI leaf samples in the baseline report, so the app process itself was not compute-bound for this run (report.md).
- The worker is dominated by one stack: `Renderer_default_renderTask -> SciVis_renderSample -> rtcIntersect8 -> brlcadIntersectN_C -> traceRay -> rt_shootray` (report.md, IBRTRenderWorker.sample.txt:87).
- Worker leaf hotspots were `rt_shootray 29.5%`, `write 12.2%`, `rt_in_rpp 10.1%`, `rt_boolfinal 6.6%`, `rt_arb_shot 5.8%`, and `rt_boolweave 4.1%` (report.md).
- Interactivity wins are available before deep renderer surgery. The transport path copies and transmits full frames even when nothing changed, and automatic mode still escalates hard views to full-resolution accumulation despite hundreds of milliseconds of frame time ([worker_main.cpp:259](../../apps/IBRT/worker_main.cpp:259), [renderworkerclient.cpp:449](../../apps/IBRT/renderworkerclient.cpp:449), launch.log:2697).
...
- [worker_main.cpp:259](../../apps/IBRT/worker_main.cpp:259): `RequestFrame` always serializes the frame header and pixel payload after `advanceRender()`, even when `updated == false`.
- [renderworkerclient.cpp:449](../../apps/IBRT/renderworkerclient.cpp:449): the client always requests a frame, allocates a `QImage`, and copies the full pixel buffer into it.
- [renderwidget.cpp:580](../../apps/IBRT/renderwidget.cpp:580): `paintGL()` performs `rgbSwapped().mirrored(false, true)` every frame before drawing.
- [renderwidget.cpp:1884](../../apps/IBRT/renderwidget.cpp:1884): the polling thread continuously requests worker frames and stores returned images.
- [renderwidget.cpp:1999](../../apps/IBRT/renderwidget.cpp:1999): the UI applies the pending image and calls `update()` without checking whether the image content actually changed.
- [ospraybackend.cpp:250](../../apps/IBRT/ospraybackend.cpp:250): `timeBudgetMs` is currently unused in `advanceRender(int)`.
- [ospraybackend.cpp:1377](../../apps/IBRT/ospraybackend.cpp:1377): the progressive ladder unconditionally advances through `16 -> 8 -> 4 -> 2 -> 1`.
- [ospraybackend.cpp:1407](../../apps/IBRT/ospraybackend.cpp:1407): full-resolution accumulation work is launched once the state machine reaches accumulate mode.
- [ospraybackend.cpp:1443](../../apps/IBRT/ospraybackend.cpp:1443): completed frames always copy pass or accumulation pixels back into display storage.
- [ospraybackend.cpp:1555](../../apps/IBRT/ospraybackend.cpp:1555): AO backoff exists, but it is reactive and only runs after slow progressive frames complete.
- [ospraybackend.h:219](../../apps/IBRT/ospraybackend.h:219): watchdog and dynamic-mode state exists but is not yet wired into an effective in-flight governor.
- [brlcad.cpp:279](../../plugins/brl_cad/geometry/brlcad.cpp:279): `traceRay()` builds one `application` and calls `rt_shootray()` for a single ray.
- [brlcad.cpp:341](../../plugins/brl_cad/geometry/brlcad.cpp:341): `brlcadIntersectN_C()` loops over packet lanes and calls `traceRay()` per lane, scalarizing Embree packets at the BRL-CAD bridge.
- [brlcad.cpp:421](../../plugins/brl_cad/geometry/brlcad.cpp:421): `numPrimitives()` exposes the BRL-CAD scene to Embree as exactly one user primitive.
- [brlcad.cpp:510](../../plugins/brl_cad/geometry/brlcad.cpp:510): `createEmbreeUserGeometry()` registers a single user-geometry object with that one primitive count.
...
- bbox.c:163: `rt_in_rpp()` is a scalar slab-style bounding-box test and a measurable leaf hotspot.
- prep.cpp:1118: `rt_get_solidbitv()` reuses or allocates bit vectors and clears them on reuse, contributing to per-ray bookkeeping overhead.
- vshoot.c:242: BRL-CAD has vector-shot hooks, but the generic `rt_vshootray()` path still bombs at the missing `boolweave` step, so it is not a near-term drop-in optimization.
- launch.log:2653: camera 2 ramps from `0.94 ms -> 2.01 ms -> 9.29 ms` in progressive passes and then climbs to `25.91 ms`, `29.67 ms`, `38.31 ms`, and `72.46 ms` in full-resolution accumulation.
- launch.log:2697: camera 3 ramps from `4.41 ms -> 13.45 ms -> 59.85 ms` in progressive passes and then spends many frames between roughly `344 ms` and `991 ms` in full-resolution accumulation.

## Recommendation Tracker

| ID | Priority | Status | Area | Expected Benefit | Evidence | Next Action | Validation |
| --- | --- | --- | --- | --- | --- | --- | --- |
| PERF-001 | P0 | ready | Frame transport | Lower IPC, fewer CPU copies, less redundant UI work, faster apparent responsiveness when the image is unchanged | [worker_main.cpp:259](../../apps/IBRT/worker_main.cpp:259), [renderworkerclient.cpp:449](../../apps/IBRT/renderworkerclient.cpp:449), [renderwidget.cpp:1999](../../apps/IBRT/renderwidget.cpp:1999) | Define a no-pixel payload path for `updated == false`, keep stats flowing, and skip repaint on unchanged frames | Reprofile worker `write`, client copy, and UI paint cost on the same `moss.g` scene |
| PERF-002 | P0 | ready | Dynamic resolution | Biggest immediate interactivity win on hard views by refusing to escalate to scale `1` too early | [ospraybackend.cpp:250](../../apps/IBRT/ospraybackend.cpp:250), [ospraybackend.cpp:1377](../../apps/IBRT/ospraybackend.cpp:1377), launch.log:2697 | Add a real scale governor in automatic mode that holds or backs off scale on slow views | Re-run the same hard camera views and confirm scale `2` or coarser is retained when frame times blow past the target |
| PERF-003 | P0 | ready | Watchdog | Prevent long blocking full-res frames and preserve responsiveness during hard views | [ospraybackend.h:219](../../apps/IBRT/ospraybackend.h:219), [ospraybackend.cpp:1407](../../apps/IBRT/ospraybackend.cpp:1407), [ospraybackend.cpp:1555](../../apps/IBRT/ospraybackend.cpp:1555) | Implement real in-flight timeout/cancel logic tied to automatic mode and reuse existing counters in telemetry | Confirm that hard views stop producing multi-hundred-millisecond stalls and that cancel counters rise in expected cases |
...
- Citations: [worker_main.cpp:259](../../apps/IBRT/worker_main.cpp:259), [renderworkerclient.cpp:449](../../apps/IBRT/renderworkerclient.cpp:449), [renderwidget.cpp:1884](../../apps/IBRT/renderwidget.cpp:1884), [renderwidget.cpp:1999](../../apps/IBRT/renderwidget.cpp:1999)
...
- Citations: [ospraybackend.cpp:250](../../apps/IBRT/ospraybackend.cpp:250), [ospraybackend.cpp:1377](../../apps/IBRT/ospraybackend.cpp:1377), [ospraybackend.cpp:1407](../../apps/IBRT/ospraybackend.cpp:1407), launch.log:2653, launch.log:2697
...
- Citations: [ospraybackend.h:219](../../apps/IBRT/ospraybackend.h:219), [ospraybackend.cpp:1407](../../apps/IBRT/ospraybackend.cpp:1407), [ospraybackend.cpp:1443](../../apps/IBRT/ospraybackend.cpp:1443), [ospraybackend.cpp:1555](../../apps/IBRT/ospraybackend.cpp:1555), launch.log:2697
...
- Citations: External OSPRay source `ospray/api/Device.cpp:104`, IBRTRenderWorker.sample.txt:24, IBRTRenderWorker.sample.txt:87
...
- Citations: [renderwidget.cpp:580](../../apps/IBRT/renderwidget.cpp:580), [renderworkerclient.cpp:449](../../apps/IBRT/renderworkerclient.cpp:449), report.md
...
- Citations: [brlcad.cpp:341](../../plugins/brl_cad/geometry/brlcad.cpp:341), [brlcad.cpp:421](../../plugins/brl_cad/geometry/brlcad.cpp:421), [brlcad.cpp:510](../../plugins/brl_cad/geometry/brlcad.cpp:510), report.md
...
- Citations: prep.cpp:1118, IBRTRenderWorker.sample.txt:20213
...
- Citations: [brlcad.cpp:341](../../plugins/brl_cad/geometry/brlcad.cpp:341), vshoot.c:242
...
- Citations: bbox.c:163, report.md, IBRTRenderWorker.sample.txt:20213
